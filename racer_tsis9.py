...
background = pygame.image.load('road.png')  # background image
pygame.mixer.init()  # initializing music component
pygame.mixer.music.set_volume(0.45)  # volume of music
pygame.mixer.music.load('racer_background.wav')  # music that will be played while playing the game
...
class Enemy(pygame.sprite.Sprite):
    # enemy car is moving.
    # enemy car can collide with player.
...
class Player(pygame.sprite.Sprite):  # Player is moving by x-axis. It can collide with coin or enemy car
    ...
    def move(self):
        ...
        if self.rect.left > 43:  # possible coordinate by x-axis is limited by yellow line
            if pressed_keys[pygame.K_LEFT]:  # move to the left direction
                self.rect.move_ip(-5, 0)
        if self.rect.right < WINDOW_WIDTH - 43:
            if pressed_keys[pygame.K_RIGHT]:  # move to the right direction
                self.rect.move_ip(5, 0)
...
coins = pygame.sprite.Group()  #special array of coin sprites
coins.add(coin_1)
